...
module.exports = function mongooseExport(schema, options) {
    // need options.headers
    if (!options.headers) throw new Error('mongooseExport requires the `headers` option');
    ...
    schema.static.findAndStreamCsv = function (query) {
        query = query || {};
        return this.find(query).stream().pipe(schema.createTransformStream());
    };
